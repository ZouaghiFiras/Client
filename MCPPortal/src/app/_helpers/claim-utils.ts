/**
 * Populates the claims table with the appropriate descriptions.
    ...
    switch (key) {
      ...
      case 'oid':
        populateClaim(
          key,
          claims[key],
          'The oid (user subObject id) is the only claim that should be used to uniquely identify a user in an Azure AD tenant.',
          claimsTable
        );
        break;
      case 'tid':
 ...
 * @param claimsObject - The claims subObject to populate
 */
function populateClaims(claim: string, value: string, description: string, claimsObject: any[]): void {
  claimsObject.push({
    claim,
    value,
    description,
...
/**
 * Transforms Unix timestamp to a date and returns a string value of that date
 * @param date - The Unix timestamp to transform
 * @returns A string representation of the transformed date
 */
